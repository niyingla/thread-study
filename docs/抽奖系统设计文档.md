...
- 建议补一个**对账/补偿定时任务**:扫 `send_status ≠ 1` 重发,并校准 Redis 已发数与 DB 记录数。
...
├── consumer/LotteryRecordConsumer      落库 + 发奖(幂等 + 补偿)
...
2. **对账/补偿定时任务**:扫 `send_status ≠ 1` 重发;Redis 已发数 vs DB 记录数校准。
